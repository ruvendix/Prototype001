// Copyright 2024 Ruvendix, All Rights Reserved.
#pragma once

class PlayerActor;
class PlayerImplementation;

class PlayerState
{
public:
	PlayerState(PlayerImplementation* pOwner)
	{
		m_pOwner = pOwner;
	}

	virtual void ProcessPlayerInput() = 0;
	virtual void UpdateState(float deltaSeconds) = 0;

	PlayerActor* BringPlayerActor() const;

	PlayerImplementation* GetOwner() const { return m_pOwner; }

private:
	PlayerImplementation* m_pOwner = nullptr;
};

class PlayerIdleState : public PlayerState
{
	using Super = PlayerState;

public:
	using Super::Super;

	virtual void ProcessPlayerInput() override;
	virtual void UpdateState(float deltaSeconds) override;
};

class PlayerWalkState : public PlayerState
{
	using Super = PlayerState;

public:
	using Super::Super;

	virtual void ProcessPlayerInput() override;
	virtual void UpdateState(float deltaSeconds) override;
};