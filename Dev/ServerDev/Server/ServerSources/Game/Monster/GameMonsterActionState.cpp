...
void GameMonsterIdleState::OnActionTimer()
{
	...
	Protocol::EntityInfo& refGameEntityInfo = spOwner->GetGameEntityInfo();
	...
	if (spFoundPlayer != nullptr)
	{
		...
		{
			refGameEntityInfo.set_entity_state(Protocol::EEntityState::Attack);
		...
		{
			refGameEntityInfo.set_entity_state(Protocol::EEntityState::Chase);
...
void GameMonsterIdleState::ProcessRandomMove()
{
	...
	int32 randMoveDirIdx = (std::rand() % Protocol::EEntityLookAtDirection_ARRAYSIZE);
	...
	refGameEntityInfo.set_entity_state(Protocol::EEntityState::Walk);
	refGameEntityInfo.set_entity_look_at_dir(static_cast<Protocol::EEntityLookAtDirection>(randMoveDirIdx));
...
void GameMonsterWalkState::OnActionTimer()
{
	...
	Protocol::EntityInfo& refGameEntityInfo = spOwner->GetGameEntityInfo();
	refGameEntityInfo.set_entity_state(Protocol::EEntityState::Idle);
...
void GameMonsterChaseState::ProcessNavigationMove(const Position2d& moveNavigationPos) const
{
	...
	Protocol::EEntityLookAtDirection lookAtDir = spOwner->CalculateGameEntityLookAtDirection(moveNavigationPos);

	Protocol::EntityInfo& refGameEntityInfo = spOwner->GetGameEntityInfo();
	refGameEntityInfo.set_entity_look_at_dir(lookAtDir);
...
void GameMonsterAttackState::OnActionTimer()
{
	...
	Protocol::EntityInfo& refVictimPlayerInfo = m_spVictimPlayer->GetGameEntityInfo();
	...
	// 공격 방향 알아내기
	Protocol::EEntityLookAtDirection attackerLookAtDir = spOwner->CalculateGameEntityLookAtDirection(victimCellPos);
	Protocol::EntityInfo& refAttackerInfo = spOwner->GetGameEntityInfo();
	refAttackerInfo.set_entity_look_at_dir(attackerLookAtDir);

	// 피해자의 피해량 계산은 공용
	int32 remainVictimPlayerHp = GameRoom::I()->CalculateVictimDamage(refAttackerInfo, refVictimPlayerInfo);
	refVictimPlayerInfo.set_hp(remainVictimPlayerHp);

	bool bPlayerDeath = (remainVictimPlayerHp <= 0);
	if (bPlayerDeath == true) // 사망
	{
		refVictimPlayerInfo.set_entity_state(Protocol::EEntityState::Death);
		ToIdleState();

		const RxSendBufferPtr& diePlayerPacket = RxServerPacketHandler::I()->MakeDieEntityPacket(refVictimPlayerInfo);
	...
	else // 공격자와 멀어진 경우는 상황에 따라 판단
	{
		...
		{
			// 추격 상태로 변경
			refAttackerInfo.set_entity_state(Protocol::EEntityState::Chase);
			spOwner->ReserveNextState<GameMonsterIdleState>();
